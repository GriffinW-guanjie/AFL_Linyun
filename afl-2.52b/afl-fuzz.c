...
/// empirical study
EXP_ST u64 total_time = 500;
EXP_ST u64 diff_branch = 10;
...
/// empirical study
static inline u8 has_new_bits_emp(u8* virgin_map) {
  u32 branch_index = 0;
  ...
  while (i--) {
    ...
    if (unlikely(*current)) {
      ...
      if(ret == 0  && unlikely(*current & *virgin)){
        ret = 1;
      }
      branch_index += 8;
#else
...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault) {
  ...
  if (fault == crash_mode) {
    ...
    if (!(hnb = has_new_bits_emp(virgin_bits))) {
